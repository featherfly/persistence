...
public class JdbcPersistenceImpl extends PersistenceObserver implements JdbcPersistence {
	 ...
	 * 具体的条件以传入对象类属性上的标注了@Pk和@Column的name获取
	 * </p>
	 * @param <E> 对象类型
	 * @param entity 对象
	 */
	...
	public <E> E find(String sql, Class<E> mappingType,
		...
		try {
			return namedParameterJdbcTemplate.queryForObject(
				sql, params, getParameterizedBeanPropertyRowMapper(mappingType));
	...
	public <E> E find(String sql, final RowMapper<E> rowMapper,
		...
		try {
			return namedParameterJdbcTemplate.queryForObject(					
				sql, params, new org.springframework.jdbc.core.RowMapper<E>() {
					@Override
	...
	public <E> E find(String sql, final RowMapper<E> rowMapper,
		...
		try {
			return jdbcTemplate.queryForObject(
					sql, new org.springframework.jdbc.core.RowMapper<E>() {
						@Override
						public E mapRow(ResultSet rs, int rowNum) throws SQLException {
							return rowMapper.mapRow(rs, rowNum);
						}
...
//		}
	 ...
	 */
	@Override
	public PaginationResults<Map<String, Object>> findPage(String sql, int start,
...
//		}
	 ...
	 */
	@Override
	public <E> PaginationResults<E> findPage(String sql, Class<E> mappingType, int start, int limit, List<Object> params) {
...
//		}
	 ...
	 */
	@Override
	public <E> PaginationResults<E> findPage(String sql, RowMapper<E> rowMapper, int start,
			int limit, Object[] params) {
		checkStartAndLimit(start, limit);
		SimplePagination<E> pagination = new SimplePagination<E>();
		List<E> results = findList(sql, rowMapper, start, limit, params);
	...
	public <E> Serializable getIdentity(E entity) {
		return simpleORMFactory
				.getSimpleORM(ClassUtils.castGenericType(entity.getClass(), entity))
				.getIdentity(entity);
	}
...
//	private <E> ParameterizedBeanPropertyRowMapper<E> getParameterizedBeanPropertyRowMapper(Class<E> mappingType) {
//		ParameterizedBeanPropertyRowMapper<E> rowMapper = new ParameterizedBeanPropertyRowMapper<E>();
...
//	}
	...
	private SimpleJdbcInsert getSimpleJdbcInsert(String tableName, String...pkColumnNames) {
		...
		if (LangUtils.isEmpty(jdbcInsert.getGeneratedKeyNames())) {
			logger.debug("表{}设置主键列{}",
					tableName, ArrayUtils.toString(pkColumnNames));
			jdbcInsert.usingGeneratedKeyColumns(pkColumnNames);
		} else {
			logger.debug("表{}已经设置了主键列{}，新的设置被忽略",
					tableName, ArrayUtils.toString(pkColumnNames));
		}
		return jdbcInsert;
	}
	...
	private Map<String, SimpleJdbcInsert> simpleJdbcInserts = new HashMap<String, SimpleJdbcInsert>(0);
